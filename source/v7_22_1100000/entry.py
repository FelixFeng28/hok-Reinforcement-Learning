...
def parse_checkpoint_models(raw_value):
    """Validate one (vs common AI) or two (head-to-head) model directories."""
    model_dirs = [item.strip() for item in raw_value.split(";") if item.strip()]
    if not model_dirs:
        return []
    if len(model_dirs) > AGENT_NUM:
        raise ValueError(
            "--checkpoint_models accepts one directory (vs common AI) or two "
            "directories (head-to-head)"
        )
    for model_dir in model_dirs:
        model_file = os.path.join(model_dir, "model.pth")
        if not os.path.isfile(model_file):
            raise ValueError(
                "checkpoint model directory must contain model.pth: {}".format(
                    model_dir
                )
            )
    return model_dirs
...
def run(
    ...
    try:
        ...
        _run(
            ...
            checkpoint_model_slot=checkpoint_model_slot,
        )
    except SystemExit:
...
def _run(
    ...
    checkpoint_eval = bool(checkpoint_models)
    if checkpoint_model_slot not in (-1, 0, 1):
        raise ValueError("--checkpoint_model_slot must be -1, 0, or 1")
    if checkpoint_model_slot >= 0 and len(checkpoint_models) != 1:
        raise ValueError(
            "--checkpoint_model_slot is only valid for one-model common-AI evaluation"
        )
    ...
    if checkpoint_eval:
        main_agent = (
            checkpoint_model_slot
            if checkpoint_model_slot >= 0
            else random.randint(0, AGENT_NUM - 1)
        )
    else:
        # Training subsequently rebalances every episode in Actor.  Starting
        # from actor parity makes the first wave balanced across actors too.
        main_agent = actor_id % AGENT_NUM

    LOG.info("load config.dat: {}", config_path)
    lib_processor = interface.Interface()
    lib_processor.Init(config_path)
    # Two network-controlled sides must have independent inference models.
    # Otherwise Model() returns the legacy Singleton and loading the sampled
    # opponent overwrites the latest-policy weights (or vice versa).  Keep the
    # shared model for common-AI training, where only one side uses a network.
    two_network_sides = (
        (checkpoint_eval and len(checkpoint_models) == 2)
        or (not checkpoint_eval and config.ENEMY_TYPE == "network")
    )
    Model = get_model_class(config.backend, shared=not two_network_sides)
    if two_network_sides:
        LOG.info("use independent model instances for two network-controlled sides")

    for i in range(AGENT_NUM):
        agents.append(
            Agent(
                Model(),
                None if checkpoint_eval else model_pool_addr.split(";"),
                config=config,
                keep_latest=(i == main_agent),
                # Explicit checkpoint evaluation must use Agent.reset(...,
                # model_path=...).  --single_test intentionally skips that
                # load path, so it cannot be used here.
                single_test=False if checkpoint_eval else single_test,
            )
        )
    ...
    mempool = (
        mem_pool_addr_list[0]
        if checkpoint_eval
        else select_mempool(actor_id, config.actor_num, mem_pool_addr_list)
    )
    sample_manager = SampleManager(
        ...
        # Evaluation never emits samples, and this avoids connecting to the
        # live learner memory pool.
        single_test=single_test or checkpoint_eval,
    ...
    actor = Actor(
        ...
        is_train=config.IS_TRAIN and not checkpoint_eval,
        enemy_type=config.ENEMY_TYPE,
        common_ai_mix_ratio=config.COMMON_AI_MIX_RATIO,
        balance_training_camps=config.BALANCE_TRAINING_CAMPS,
    )
    ...
    try:
        if checkpoint_eval:
            LOG.info(
                "CHECKPOINT_EVAL: models={}, simulator=remote_repeat",
                checkpoint_models,
            )
            LOG.info("CHECKPOINT_EVAL: network_model_slot={}", main_agent)
            # Keep deterministic evaluation actions and sample isolation, but
            # use the same RemoteRepeat GameCore mode as the live actors.
            actor.run(
                load_models=checkpoint_models,
                eval_freq=1,
                gamecore_eval=False,
                force_eval=True,
            )
...
def main(_):
    FLAGS = flags.FLAGS
    checkpoint_models = parse_checkpoint_models(FLAGS.checkpoint_models)
    if checkpoint_models and FLAGS.single_test:
        raise ValueError(
            "--checkpoint_models loads explicit model.pth files; do not combine it "
            "with --single_test"
        )
    ...
    run(
        ...
        Config,
        checkpoint_models=checkpoint_models,
        checkpoint_model_slot=FLAGS.checkpoint_model_slot,
        log_file=FLAGS.log_file or None,
...
if __name__ == "__main__":
    ...
    flags.DEFINE_string(
        "checkpoint_models",
        "",
        "one model directory for common-AI evaluation, or two semicolon-separated "
        "model directories for head-to-head evaluation",
    )
    flags.DEFINE_integer(
        "checkpoint_model_slot",
        -1,
        "for one-model common-AI evaluation: force the network model into slot 0 or 1",
    )
    flags.DEFINE_string(
        "config_path",
    ...
    flags.DEFINE_string("log_file", "", "override actor log file path")
