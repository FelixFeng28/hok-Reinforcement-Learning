...
def main(_):
    ...
    # Saving is still additionally bounded by ``save_model_seconds`` in the
    # framework.  This override only makes the step-based cadence explicit
    # for short, checkpoint-gated stabilization experiments.
    save_model_steps = os.getenv("AIARENA_SAVE_MODEL_STEPS")
    if save_model_steps is not None:
        try:
            config_manager.save_model_steps = int(save_model_steps)
        except ValueError as exc:
            raise ValueError(
                "AIARENA_SAVE_MODEL_STEPS must be a positive integer"
            ) from exc
        if config_manager.save_model_steps <= 0:
            raise ValueError(
                "AIARENA_SAVE_MODEL_STEPS must be a positive integer"
            )

    LOG.info(
        "learner overrides: learning_rate={}, ppo_clip={}, save_model_steps={}",
        Config.INIT_LEARNING_RATE_START,
        Config.CLIP_PARAM,
        config_manager.save_model_steps,
    )
